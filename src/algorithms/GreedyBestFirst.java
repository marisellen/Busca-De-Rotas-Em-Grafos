...
public class GreedyBestFirst {
    ...
    public void executar(int xOrig, int yOrig, int xDest, int yDest) {
        ...
        PriorityQueue<Node> fila = new PriorityQueue<>(Comparator.comparingDouble(n -> heuristica.calcular(n, destino)));
        ...
        while(!fila.isEmpty()) {
            ...
            for(Node vizinho : gerarVizinhos(atual)) {
                if(!visitados.contains(vizinho)) {
                    vizinho.pai = atual;
                    vizinho.custo = atual.custo + grafo.getPeso(atual.x, atual.y, vizinho.x, vizinho.y);
                    fila.add(vizinho);
                }
            }
        }
    ...
    public void gerarArquivoSaida(String arquivoEntrada, String heur) {
        try {
            ...
            pw.println("ALGORITIMO: Greedy Best-First Search");
