...
public class DFS {
    ...
    private long tempo;
        ...
        public void executar(int xOrig, int yOrig, int xDest, int yDest) {
            ...
            while(!pilha.isEmpty()) {
                ...
                for(Node vizinho : gerarVizinhos(atual)) {
                    if(!visitados.contains(vizinho)) pilha.push(vizinho);
                }
            }
        ...
        private List<Node> gerarVizinhos(Node atual) {
            ...
            for(int i=0;i<4;i++) {
                ...
                if(peso > 0) vizinhos.add(new Node(nx, ny, atual.custo + peso, atual));
            }
            return vizinhos;
        }
    ...
    public void gerarArquivoSaida(String arquivoEntrada) {
        try {
            ...
            File arquivoSaida = new File(dir, baseNome + ".dfs");

            PrintWriter pw = new PrintWriter(arquivoSaida);

            pw.println("ALGORITIMO: DFS");
